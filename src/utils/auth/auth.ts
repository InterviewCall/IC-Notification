...
const { SALT, JWT_SECRET, JWT_EXPRIRES_IN } = serverConfig;
...
function createToken(payload: JwtTokenInput): string {
    try {
        const token = jwt.sign(payload, JWT_SECRET as Secret, { expiresIn: JWT_EXPRIRES_IN as jwt.SignOptions['expiresIn'] });
        return token;
    } catch (error) {
        throw error;
    }
